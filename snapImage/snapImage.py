...
class snapImage:
    ...
    def location(self,img):
        ...
        #white_light_lower  = np.array([56,13,184])
        #white_light_upper = np.array([76,33,264])
        #white_light_mask = cv2.inRange(img, white_light_lower,white_light_upper)
        #self.find_contour(white_light_mask)# [ -1 199  72] [ 19 219 152]

        #yellow_light_lower  = np.array([20,245,88])
        #yellow_light_upper = np.array([40,265,168])
        #yellow_light_mask = cv2.inRange(img, yellow_light_lower,yellow_light_upper)
        #self.find_contour(yellow_light_mask)# [ -1 199  72] [ 19 219 152]
        ...
        green_lower  = np.array([20,245,88])
        green_upper = np.array([40,265,168])
        ...
        #red_lower = np.array([-10,241,204])
        #red_upper = np.array([10,261,284])
    ...
    def dataCollection(self):
        ...
        cv2.imwrite("SD/"+str(self.folderName)+"/"+str(self.imgID)+".png",SD_image)
        time.sleep(1)
        ...
        cv2.imwrite("cantoon/"+str(self.folderName)+"/"+str(self.imgID)+".png",Cantoon_image)
        self.imgID +=1
        time.sleep(1)
        pyautogui.press('f5')
        # get back to SD    
